...
class VirusTotalFetcher:
    ...
    def get_previous_query(self):
        try:
            ...
            if self.previous_vt_query:
                self.indicator.vt_indications = self.previous_vt_query.get(
                    "vt_indications"
                )
                self.indicator.creation_date = self.previous_vt_query.get(
                    "creation_date"
                )
                ...
                self.indicator.days_since_creation = self.previous_vt_query.get(
                    "days_since_creation"
                )
                self.indicator.days_since_last_scanned = self.previous_vt_query.get(
                    "days_since_last_scanned"
                )
        ...
        if self.indicator.rescan_id:
            ...
            while scan_complete is False:
                ...
                if str(response.status_code).startswith("2"):
                    ...
                    status = (
                        self.decoded_response.get("data", {})
                        .get("attributes", {})
                        .get("status", "")
                    )
                    if status == "completed":
    ...
    def assign_results(self):
        try:
            ...
            # else:
            self.logger.info("Attributing VT data")
            data_response = self.decoded_response.get("data", {})
            filtered_response = data_response.get("attributes", {})
            ...
            self.indicator.last_scanned = filtered_response.get(
                "last_analysis_date", ""
            )
            ...
            if self.indicator.creation_date:
                ...
                self.indicator.days_since_creation = (
                    self.today - creation_datetime
                ).days
            else:
                self.indicator.days_since_creation = 100

            if self.indicator.last_scanned != "-":
                last_scanned_datetime = datetime.utcfromtimestamp(
                    self.indicator.last_scanned
                )
                self.indicator.days_since_last_scanned = (
                    self.today - last_scanned_datetime
                ).days
            else:
                self.indicator.days_since_last_scanned = 365
        except Exception as e:
            self.logger.error(
                f"Error attributing VT data to {self.indicator.fqdn}: {e}"
            )
            self.no_data()
            raise
